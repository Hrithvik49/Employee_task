...
from employee_task.utils.mail import send_mail_async as send_mail
    ...
    def send_new_task_email(self):
        ...
        if len(emails_to):
            ...
            if settings.TASKS_VIEWER_ENABLED:
                email_template = settings.TASKS_EMAIL_WITH_URL
                vals["url"] = self.get_tasks_viewer_url()
            else:
                email_template = settings.TASKS_EMAIL_WITHOUT_URL
            try:
                send_mail(
